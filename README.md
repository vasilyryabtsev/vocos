...
Скрипт ожидает аудиофайлы в поддиректории `audio/` внутри `data_dir` (поддерживаются `.wav`, `.mp3`, `.flac`, `.m4a`). Результаты сохраняются в `output_dir` с исходными именами файлов.

```
data_dir/
  audio/           ← аудиофайлы для ресинтеза
  transcriptions/  ← (опционально) текстовые транскрипции (.txt)
```

**Параметры:**
| Параметр | По умолчанию | Описание |
|---|---|---|
| `data_dir` | обязательный | Директория с аудиофайлами и транскрипциями к ним|
...
| `resynthesize` | `true` | Запустить ресинтез аудиофайлов, иначе запускается полный TTS пайплайн |
